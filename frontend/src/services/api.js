import axios from 'axios';

const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://localhost:3001/api';

// Create axios instance
const api = axios.create({
  baseURL: API_BASE_URL,
  headers: {
    'Content-Type': 'application/json',
  },
});

// Add request interceptor to include auth token
api.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('token');
    if (token) {
      config.headers.Authorization = `Bearer ${token}`;
    }
    return config;
  },
  (error) => {
    return Promise.reject(error);
  }
);

// Add response interceptor to handle errors
api.interceptors.response.use(
  (response) => response,
  (error) => {
    if (error.response?.status === 401) {
      localStorage.removeItem('token');
      localStorage.removeItem('user');
      window.location.href = '/login';
    }
    return Promise.reject(error);
  }
);

// Auth API
export const authAPI = {
  login: (credentials) => api.post('/auth/login', credentials),
  register: (userData) => api.post('/auth/register', userData),
  getCurrentUser: () => api.get('/auth/me'),
};

// Messages API
export const messagesAPI = {
  sendMessage: (messageData) => api.post('/messages', messageData),
  getMessages: (type) => api.get(`/messages${type ? `?type=${type}` : ''}`),
  getMessage: (id) => api.get(`/messages/${id}`),
  getConversation: (userId) => api.get(`/messages/conversation/${userId}`),
  getAllUsers: () => api.get('/messages/users/all'),
  deleteMessage: (id) => api.delete(`/messages/${id}`),
};

// Maintenance API
export const maintenanceAPI = {
  createRequest: (requestData) => api.post('/maintenance', requestData),
  getAllRequests: (params) => api.get('/maintenance', { params }),
  getMyRequests: () => api.get('/maintenance/my-requests'),
  getRequest: (id) => api.get(`/maintenance/${id}`),
  updateStatus: (id, statusData) => api.patch(`/maintenance/${id}/status`, statusData),
  addUpdate: (id, notes) => api.post(`/maintenance/${id}/updates`, { notes }),
  deleteRequest: (id) => api.delete(`/maintenance/${id}`),
};

export default api;
